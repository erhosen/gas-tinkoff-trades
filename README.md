...
* Сохранить скрипт 💾
* В документе Google Spreadsheets выберите ячейку по вашему усмотрению и присвойте ей имя `UPDATE_DATE` с помощью меню `Data`->`Named ranges`->`Add named range`. В эту ячейку по команде меню TI->Обновить вставляется текущая дата. Данная ячейка может используется в качестве необязательного параметра `dummy` любой функции для [принудительного обновления](https://stackoverflow.com/a/27656313).
