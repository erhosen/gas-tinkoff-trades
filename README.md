...
* Скрипт резервирует ячейку `Z1` (самая правая ячейка первой строки), в которую вставляет текущая дата. Данная ячейка может используется в качестве необязательного параметра любой функции для [принудительного обновления](https://stackoverflow.com/a/27656313).
